...
def single_foil(mat, node, pk_data, t_w, mass):
    nodal_response = []
    # load data
    name = 'data/' + mat + str(node) + '.csv'
    data = np.loadtxt(name, delimiter=',', skiprows=1)
    erg, counts = data.T[1], data.T[4]
...
def single_node(materials, node, wait_times, masses):
    ...
    peak_data['aucd'] = ((412, 2.7*3600*24, 0.95), (356, 6.18*3600*24, 0.94))  # keV, s
...
def store_all_response_vectors():
    ...
    for i in range(4):
        r = single_node(['in', 'incd', 'rh', 'al'], i+1, [1200, 1200, 1200, 1200], masses[i])
        np.savetxt('response_vectors/experiment_1_node_{}.txt'.format(i+1), r)
